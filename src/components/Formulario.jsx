...
const [form, setform] = useState({
    ...
    const handleSubmit = async(e)=>{
        ...
        try {
            if(form.id){
                const url = `https://65b95966b71048505a8ab5d8.mockapi.io/Metro/${form.id}`
                await fetch(url,{
                    method:'PUT',
            ...
            const url ="https://65b95966b71048505a8ab5d8.mockapi.io/Metro"
						form.id = uuidv4()
            await fetch(url,{
                method:'POST',
                body:JSON.stringify(form),
    ...
    useEffect(() => {
        if(idMetro)
        {
            (async function (idMetro) {
                try {
                    const respuesta = await (await fetch(`https://65b95966b71048505a8ab5d8.mockapi.io/Metro/${idMetro}`)).json()
