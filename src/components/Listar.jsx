...
const Listar = ({ estado,setIdmetro}) => {
  ...
  useEffect(() => {
    if(estado || rutas.length>=0)
    {
        (async function () {
            try {
                const respuesta = await (await fetch("https://65b95966b71048505a8ab5d8.mockapi.io/Metro")).json()
                setRutas(respuesta)
                //setIdmetro(0)--> Otra forma de hacer 
...
const handleDelete = async (id) => {
    try {
        ...
        if (confirmar) {
            const url = `https://65b95966b71048505a8ab5d8.mockapi.io/Metro/${id}`
            await fetch(url, {
                method: 'DELETE',
            })
